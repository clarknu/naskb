...
Vue3 全局构建版（`public/vendor/vue.global.prod.js`）+ 原生 JS/CSS，**无打包步骤**：
`public/` 即服务托管的生产目录（`naskb desc serve-platform` 直接挂载），
...
本 `public/` 即可，服务端零改动（platform-v3-design §6.1 决策 2）。
（注：目录不叫 `dist/` 是为避开仓库历史 `.gitignore` 的 `dist/` 规则。）
