...
description: |
  ...
  触发词消歧：与 desktop-ui-design 共享“页面设计”等通用词，命中此类词时按端类型路由——
  移动端/小程序/App → 本 skill；PC/桌面/后台 → desktop-ui-design。
triggers:
  - 移动端设计
  - 小程序设计
  - App 设计
...
lineage:
  origin: arb-hub
  sources:
    boxing:        {sha256: 821d3d45bdc4}
    zy-iot-ai:     {sha256: 6626dcfc12e0}
    zy-ai-consult: {sha256: 6626dcfc12e0}
---
...
| 字段 | 必需 | 类型 | 说明 |
|------|------|------|------|
| `type` | 是 | `"page"` / `"zone"` / `"component"` | 节点类型（Tab 层为隐式根节点，不加 type）。功能组件统一为 `"component"`（早期示例中的 `"func"` 为旧写法，已废弃） |
...
| 输入 | 来源 | 说明 |
|------|------|------|
| 当前设计文件 | `design/06-{client-slug}/data/tree.js` | 待检查的页面功能结构 |
