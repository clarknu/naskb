...
window.SFDS_DATA["pipeline-registry"] = (function () {
  ...
  return {
    ...
    skills: [
      ...
      { name: "requirements-backlog", layer: "基础", stage: "bypass", priority: 0,
        triggers: ["先记下来", "记一下这个想法", "先放着", "暂存", "记到待办", "这个需求先不急", "中远期", "回顾下待办", "看看哪些能拎起来", "哪条该做", "梳理下待办", "细化一下这个需求", "纳入方法论", "从 XX 需求开始设计", "从头开始迭代设计", "requirements backlog", "backlog review"],
        summary: "需求积压/中远期暂存机制（CASE-032）——把未排期研发想法先捕获→暂存中远期→回顾→细化→纳入主链/iterate 从头迭代设计；只维护 docs/requirements-backlog.md，不碰 design/src；与 consolidate-raw-input（向后整理）正交、与 iterate（下游）分工",
        inputs: ["未排期研发需求/想法"], outputs: ["docs/requirements-backlog.md 状态推进"],
        upstream: ["development-standard"], downstream: ["business-workflow", "iterate"] },
