...
window.SFDS_DATA["pipeline-registry"] = (function () {
  ...
  return {
    ...
    stages: [
      ...
      { id: "ui",         name: "页面设计",   order: 6, skills: ["mobile-app-design", "desktop-ui-design"], optional: "按端类型", exitGates: [{ name: "功能树就绪", check: "design/{NN}-{client}/data/tree.js + loader 登记" }] },
      { id: "tdd",        name: "TDD 设计",   order: 7, skills: ["tdd-build"], exitGates: [{ name: "测试设计 + 代码就绪", check: "design/07-tdd/ 设计文档 + tests/ 代码（契约先行）" }] },
      { id: "impl",       name: "代码实现",   order: 8, skills: ["api-code-gen", "mobile-code-gen", "desktop-code-gen"], exitGates: [{ name: "后置一致性检查零差异", check: "各 code-gen skill 步骤 6 / 一致性检查模式" }, { name: "架构契约退出码 0（L2+）", check: "tests/test_arch_contract.py" }] },
    ...
    skills: [
      ...
      { name: "mobile-app-design", layer: "设计", stage: "ui", priority: 2,
        triggers: ["移动端设计", "小程序设计", "页面设计"],
        summary: "移动端页面功能设计（§8.4），design-viewer + data/*.js（TabBar 根）",
        inputs: ["workflow", "er", "api"], outputs: ["design/{NN}-{client}/data/tree.js 等"],
        upstream: ["business-workflow", "entity-relationship"], downstream: ["mobile-code-gen", "tdd-build"] },
      { name: "desktop-ui-design", layer: "设计", stage: "ui", priority: 2,
        triggers: ["PC 端设计", "桌面端设计", "后台设计", "Web 后台设计"],
        summary: "桌面/Web 端页面功能设计（§8.4），design-viewer + data/*.js（根节点展开）",
        inputs: ["workflow", "er", "api"], outputs: ["design/{NN}-{client}/data/tree.js 等"],
        upstream: ["business-workflow", "entity-relationship"], downstream: ["desktop-code-gen", "tdd-build"] },
      { name: "tdd-build", layer: "验证", stage: "tdd", priority: 3,
        ...
        upstream: ["api-design", "backend-architecture-design", "mobile/desktop-ui-design"], downstream: ["tdd-execute"] },
      ...
      { name: "mobile-code-gen", layer: "实现", stage: "impl", priority: 4,
        triggers: ["移动端代码生成", "前端代码"],
        summary: "移动端代码生成（§8.7）——tree.js → 页面骨架 + 一致性校核",
        inputs: ["mobile-app-design tree.js", "api"], outputs: ["移动端代码", "一致性检查报告"],
        upstream: ["mobile-app-design"], downstream: ["tdd-execute", "review"] },
      { name: "desktop-code-gen", layer: "实现", stage: "impl", priority: 4,
        triggers: ["桌面端代码生成", "后台代码", "Web 前端代码"],
        summary: "桌面/Web 端代码生成（§8.7）——tree.js → 页面骨架 + 一致性校核",
        inputs: ["desktop-ui-design tree.js", "api"], outputs: ["桌面/Web 端代码", "一致性检查报告"],
        upstream: ["desktop-ui-design"], downstream: ["tdd-execute", "review"] },
